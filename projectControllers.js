...
exports.getProjects = async (req, res) => {
  try {
    const projects = await Project.find({ owner: req.user.id }).populate('team');
    res.json(projects);
  } catch (err) {
    console.error(err);
...
exports.createProject = [
  ...
  body('team').optional().isArray().withMessage('Team must be an array of user IDs'),
  ...
  async (req, res) => {
    ...
    try {
      const project = await Project.create({ title, description, team, owner: req.user.id });
